...
def parse_sheet(path: Path, sheet_name: str, col_name: str) -> pd.DataFrame:
    ...
    # Keep only year columns in range
...
def load_panel() -> pd.DataFrame:
    ...
    print(f"  Merged panel (before cleaning): {len(panel):,} rows, "
...
def clean_and_engineer(panel: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    # Sort for time-series derivation
    panel = panel.sort_values(["country_code", "year"]).copy()

    # Derive GDP-per-capita growth (% YoY within each country).
    # This is the variable used in the falsifiable hypothesis — it aligns
    # the charter hypothesis ("GDP per capita growth") with both the data
    # source (NY.GDP.PCAP.CD) and the feature set.
...
def test_hypothesis(panel: pd.DataFrame) -> dict:
    ...
    """
    valid = panel.dropna(subset=["gdp_per_capita_growth"])
...
# ---------------------------------------------------------------------------
# 8. Plots (optional — skipped gracefully if matplotlib absent)
# ---------------------------------------------------------------------------
...
def save_plots(y_test, y_pred, baseline_pred, panel, feature_importance, model_name):
    try:
        ...
        ax.set_title("Unemployment rate by GDP per capita quartile\n"
...
def main():
    ...
    print(f"  Difference           : {hyp['difference_pp']:.2f} pp  "
    ...
    for row in strat:
        print(f"  {row['quartile']:<12}  n={row['n']:>4}  "
    ...
    write_json(OUTPUTS / "milestone_manifest.json", {
        ...
        "course":              "ECO 6810",
        "status":              "complete",
        "data_source":         "World Bank WDI — data/Book.xlsx (downloaded 2026-04-08)",
        "data_note":           "gdp_per_capita_growth derived as YoY pct_change of gdp_per_capita within each country",
    ...
    # 11. Plots
    print("\n[9] Generating plots ...")
    save_plots(y_test, y_pred, baseline["predictions"],
               panel, feat_imp, best_name)
    ...
    print(f"  Hypothesis       : {'Supported' if hyp['hypothesis_supported'] else 'Not supported'} "
