...
def compute_importance(
    best_model, X_train: np.ndarray, X_test: np.ndarray,
    y_test: np.ndarray, feature_cols: list[str]
) -> dict:
    """
    Three importance methods to check stability of feature rankings.

    Gini (MDI): fast, from training splits; known to favour high-cardinality.
    Permutation: model-agnostic, computed on test set; more reliable estimate
                 of real predictive contribution.
    Bootstrap:  Gini across 5 bootstrap resamples of training data;
                shows rank variance across different data draws.
    """
    print("\n[6] Feature importance — three methods ...")
    ...
    # Permutation importance on test set (n_repeats=10)
